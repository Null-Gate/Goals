...
pub async fn login(info: Json<LoginInfo>) -> HttpResponse {
    ...
    match db.select::<Option<UserInfo>>(("user", &info.username)).await {
        Ok(Some(user)) => {
        ...
        Ok(None) => HttpResponse::NotFound().json(Resp::new(&format!("Sorry No User is founded with username of: {}", info.username))),
        Err(e) => {
            HttpResponse::InternalServerError().json(Resp::new(&format!("Sorry We are some problem in opening database!!: {:?}", e)))
        }
    }
}
